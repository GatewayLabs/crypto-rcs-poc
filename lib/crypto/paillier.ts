...
export class PaillierCrypto {
  ...
  generateProof(m: number, r: bigint): {
    commitment: string;
    challenge: string;
    response: string;
  } {
    ...
    const challengeInput = ethers.utils.solidityPack(
      ['uint256', 'uint256'],
      [commitment, BigInt(m)]
    );
    const challengeHash = ethers.utils.keccak256(challengeInput);
  ...
  verifyProof(
    ...
    const right = (commitment * modPow(c, challenge, this.nSquared)) % this.nSquared;
